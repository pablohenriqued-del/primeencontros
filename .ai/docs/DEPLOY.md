...
O backend usava um pacote (`emergentintegrations==0.2.0`) que **não existe no PyPI público** — `pip install -r requirements.txt` falhava fora do ambiente Emergent. Isso foi resolvido nesta mesma leva: o wrapper de Stripe foi trocado pelo SDK oficial `stripe` (ver `.ai/context/CONTRACTS.md` #9). **O Object Storage da Emergent (fotos/vídeo, `EMERGENT_LLM_KEY`) foi mantido de propósito** nessa leva — e foi exatamente essa dependência adiada que quebrou o upload em produção. Resolvido em 2026-07-18: mídia agora fica em disco local (volume Docker `uploads_data`, ver CONTRACTS.md #8/#10). O backend ainda precisa de egress HTTPS liberado para `demobackend.emergentagent.com` (auth de login via Google — isso não mudou).
...
cp backend/.env.example backend/.env        # DATABASE_URL, STRIPE_API_KEY, STRIPE_WEBHOOK_SECRET, ADMIN_EMAILS
```
...
| `backend/.env` | `STORAGE_DIR` | Opcional | Onde ficam fotos/vídeo em disco (default `/data/uploads`). Só mudar se souber o que está fazendo — precisa bater com o mountpoint do volume `uploads_data` no `docker-compose.yml`. |
...
- Checkout (Stripe) foi migrado/revisado no código e testado o quanto dava sem credenciais reais — mas não foi exercitado ponta-a-ponta neste ambiente (sem `sk_test_...` real). Validar manualmente na VPS com a chave de verdade antes de considerar 100% confirmado em produção. Upload de mídia não depende mais de credencial externa (ver seção 3).
