...
4. ~~Object Storage ainda depende da Emergent~~ **RESOLVIDO em 2026-07-18**: upload de fotos/vídeo migrado pra disco local (volume Docker `uploads_data`, env `STORAGE_DIR`) — ver CONTRACTS.md #8/#10. Era a causa do upload quebrado em produção (`EMERGENT_LLM_KEY` nunca validada fora do ambiente Emergent). A VPS ainda precisa de egress HTTPS pra `demobackend.emergentagent.com` (auth de login, isso continua Emergent — não mexido).
...
9. **Checkout não testado ponta-a-ponta**: migrado/revisado no código, mas sem credenciais reais (Stripe `sk_test_...`) neste ambiente pra validar de verdade. Confirmar manualmente na VPS com a chave real antes de considerar 100% pronto. (Upload de mídia não depende mais de credencial externa — ver item 4.)
